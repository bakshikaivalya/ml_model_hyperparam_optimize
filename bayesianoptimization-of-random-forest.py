"""
Algorithm to optimize the hyper-parameters of multi-class, multi-output classifiers using Bayesian optimization (BO).
The example here implements a random forest classifier but can be used for optimal training of any general classifier
by tuning the hyper-parameters in an intelligent fashion rather than relying on old-fashioned and manual methods such as
grid or random search. Gaussian process regression and Bayesian learning have been combined in an elegant way
(https://github.com/fmfn/BayesianOptimization) which can be usd to perform the explore-exploit trade-off in parameter
selection effectively. Below is the generalization of the basic idea
(https://www.kaggle.com/mpearmain/bayesianoptimization-of-random-forest) to the multi-output classification case.
...
class BayesianOptimization(object):
    ...
    def maximize(self, init_points=5, restarts=50, n_iter=25, acq='ei', **gp_params):
        ...
        # only theta0.
        gp = GP()  # TODO: arguments not accepted in scikit-learn v 0.21.2 (theta0=numpy.random.uniform(0.001, 0.05, self.dim), random_start=25)
...
X, Y, Xtarget, Ytarget=model_selection.train_test_split(train, target, test_size=0.1)
del train
